...
function solveStoryProblem() {
    ...
    setTimeout(() => {
        ...
        if (solution.success) {
            if (solution.isConversation) {
                // Conversational response (not a math problem)
                addNlpBotMessage(solution.response);
            } else {
            ...
            addNlpBotMessage(solution.error);
        }
    }, 800 + Math.random() * 600);
}
...
/**
 * Add a bot message with rich formatting (supports **bold**, \n newlines, • bullets)
 */
function addNlpBotMessage(content) {
    const chat = document.getElementById('nlpChat');
    ...
    // Newlines
    html = html.replace(/\\n/g, '<br>');
    // Bullet points (• already works as text)
    
    const msgDiv = document.createElement('div');
    msgDiv.className = 'nlp-message nlp-bot';
    msgDiv.innerHTML = `
