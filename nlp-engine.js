...
class ElhitungNLP {
    ...
    solve(text) {
        try {
            ...
            if (rawNumbers.length === 0) {
                return this.errorResult('Tidak ditemukan angka dalam soal. Pastikan soal mengandung angka yang jelas.', text);
            }
        ...
        for (const pattern of this.questionPatterns) {
            if (pattern.regex.test(text)) {
                return {
                    ...
                    unit: pattern.qunit,
                    text: questionText,
                    startIndex: questionMatch ? text.indexOf(questionMatch[0]) : -1
                };
            }
        }

        return { type: 'general', unit: null, text: questionText, startIndex: -1 };
    }
    ...
    solveStandardOperation(originalText, normalized, operands, question) {
        ...
        const unit = operands.find(n => n.unit && !n.isPercentage)?.unit || '';
